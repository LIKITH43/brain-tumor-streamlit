...
def extract_features(img: Image.Image) -> np.ndarray:
    ...
    Uses: intensity stats, edge density, Laplacian variance, 
          quadrant asymmetry, and histogram bins.
    ...
    # 1. Global stats
    feats = [
        norm.mean(), norm.std(), np.percentile(norm, 25),
        np.percentile(norm, 75), norm.max() - norm.min(),
    ]
    ...
    # 3. Laplacian variance (blur / sharpness proxy)
    lap = cv2.Laplacian(gray.astype(np.uint8), cv2.CV_64F)
    feats.append(lap.var())

    # 4. Quadrant asymmetry (tumors break symmetry)
    h, w = gray.shape
    ...
    # 6. Centre-vs-surround brightness ratio
    cx, cy = w//4, h//4
    centre = norm[cy:3*cy, cx:3*cx].mean()
    surround = norm.mean()
    feats.append(centre / (surround + 1e-6))

    # 7. Gradient magnitude mean
...
def load_model():
    ...
    def synthetic_features(label: int, n: int) -> np.ndarray:
        ...
        higher asymmetry, brighter central region, and distinct histogram shape.
        """
        base = rng.normal(0, 1, (n, 30))
        if label == 1:   # tumor
            base[:, 5]  += 3.0   # edge density
            base[:, 6]  += 4.0   # Laplacian var
            base[:, 7]  += 2.0   # asymmetry
            base[:, 8]  += 2.0
            base[:, 27] += 2.5   # centre brightness
        else:            # no tumor
            base[:, 5]  -= 0.5
            base[:, 6]  -= 0.5
        return base
